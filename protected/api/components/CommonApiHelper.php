...
class CommonApiHelper
{
    ...
    public static function return_success_response($messageTitle = '', $message = '', $data = [], $extraResponse = []) {
        $amReponseParam = CommonApiHelper::replacenull($data);
        $amResponse = array(
				'status' => "1",
				'message' => array(
					...
					'color_code' => Yii::$app->params['MESSAGE_COLOR']['success']
				),
    ...
    public static function return_error_response($messageTitle = '', $message = '', $error_flg = "0", $messageColor = '') {
        $amResponse = array(
			'status' => (string) $error_flg,
			'message' => array(
				'message_title' => $messageTitle,
				'message_content' => $message,
				'color_code' => !empty($messageColor) ? $messageColor: Yii::$app->params['MESSAGE_COLOR']['error']
			),
			'data' => []
		);
        return $amResponse;
    ...
    public static function short_big_number($value){
        ...
        if($value>=10000) return '10k mi+';
        if($value>1000) return round(($value/1000),1).'k mi';
    ...
    switch ( $info[2] ) {
      case IMAGETYPE_GIF:  $output .= "gif"; imagegif($image_resized, $path.$output);    break;
      case IMAGETYPE_JPEG: $output .= "jpeg"; imagejpeg($image_resized, $path.$output, $quality);   break;
      case IMAGETYPE_PNG:
        $output .= "png"; 
    ...
    public static function sendAPNS($ssDeviceToken, $amAPNSReques, $ssTags = 201)
    {
        ...
        if (file_exists($ssCertifiateFilePath))
        {
            ...
            $oFp = stream_socket_client($ssApnsHost, $err,
